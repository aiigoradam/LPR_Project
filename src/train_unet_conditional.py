...
from models.unet_conditional import NoiseEncoder, CondUNet, FullModel
...
def train(
    ...
    with mlflow.start_run(run_name=cfg.run_name) as run:
        ...
        mlflow.log_artifact("models/unet_conditional.py", artifact_path="code")
...
def main():
    ...
    noise_encoder = NoiseEncoder(in_ch=3, base=cfg.filters, z_dim=128).to(device)
    restorer = CondUNet(in_ch=3, out_ch=3, base=cfg.filters, z_dim=128).to(device)
    model = FullModel(noise_enc=noise_encoder, restorer=restorer).to(device)
    optimizer = optim.AdamW(
        list(noise_encoder.parameters()) + list(restorer.parameters()),
